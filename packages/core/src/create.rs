...
impl<'b> VirtualDom {
    ...
    fn load_scope_from_vcomponent(&mut self, component: &VComponent, dom: Rc<RefCell<VirtualDom>>) -> ScopeId {
        component
            .props
            .take()
            .map(|props| {
                let unbounded_props: Box<dyn AnyProps> = unsafe { std::mem::transmute(props) };
                self.new_scope(unbounded_props, component.name, dom).id
            })
...
impl<'b> Subtree {  
    ...
    pub(crate) fn create(&mut self, node: &'b VNode<'b>) -> usize {
        ...
        dom.register_template(node.template.get());
    ...
    fn write_dynamic_root(&mut self, template: &'b VNode<'b>, idx: usize) -> usize {
        ...
        match &template.dynamic_nodes[idx] {
            ...
            Placeholder(VPlaceholder { id }) => {
                ...
                self.get_dom().mutations.push(CreatePlaceholder { id });
                1
            }
            Text(VText { id, value }) => {
                let id = self.set_slot(template, id, idx);
                self.get_dom().create_static_text(value, id);
                1
            }
        }
    }
    ...
    fn load_placeholders(
        ...
        for idx in reversed_iter {
            ...
            if m > 0 {
                ...
                self.dom.borrow_mut().mutations.push(ReplacePlaceholder { m, path });
            }
        }
    }
    ...
    fn write_attrs_on_root(
            ...
            attrs.next_if(|(_, p)| p.first().copied() == Some(root_idx))
        {
            ...
            loop {
                self.dom.borrow_mut().write_attribute(&node.dynamic_attrs[attr_id], id);
    ...
    fn load_template_root(&mut self, template: &VNode, root_idx: usize) -> ElementId {
        ...
        self.dom.borrow_mut().mutations.push(LoadTemplate {
    ...
    fn assign_static_node_as_dynamic(
        ...
        self.dom.borrow_mut().mutations.push(Mutation::AssignId {
            path: &path[1..],
            id,
        });
    ...
    fn create_dynamic_text(
        ...
        self.dom.borrow_mut().mutations.push(HydrateText {
            id: new_id,
    ...
    pub(crate) fn create_placeholder(
        ...
        self.dom.borrow_mut().mutations.push(AssignId {
            path: &template.template.get().node_paths[idx][1..],
            id,
        });
    ...
    pub(super) fn create_component_node(
        ...
        let scope = dom.load_scope_from_vcomponent(component, self.dom.clone());

        component.scope.set(Some(scope));

        let to_match = unsafe { dom.run_scope(scope).extend_lifetime_ref() };
    ...
    fn mount_component(
        ...
        // is encountered
        let mutations_to_this_point = self.dom.borrow_mut().mutations.edits.len();
        ...
        if self.get_dom().collected_leaves.is_empty() {
            return created;
        }

        // If running the scope has collected some leaves and *this* component is a boundary, then handle the suspense
        let boundary = match self.get_dom().scopes[scope].has_context::<Rc<SuspenseContext>>() {
        ...
        self.get_dom().scopes[scope].placeholder.set(Some(new_id));
        ...
        let split_off = unsafe {
            std::mem::transmute::<Vec<Mutation>, Vec<Mutation>>(
                self.get_dom().mutations.edits.split_off(mutations_to_this_point),
            )
        };
        ...
        boundary
            .waiting_on
            .borrow_mut()
            .extend(self.get_dom().collected_leaves.drain(..));

        // Now assign the placeholder in the DOM
        self.get_dom().mutations.push(AssignId {
            id: new_id,
    ...
    fn mount_aborted(&mut self, parent: &'b VNode<'b>, placeholder: &VPlaceholder) -> usize {
        ...
        self.dom.borrow_mut().mutations.push(Mutation::CreatePlaceholder { id });
        placeholder.id.set(Some(id));
        1
    }
